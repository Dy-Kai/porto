// functions
function CreateGalleryImages(imgName, imgDir) {
...
async function CreateGalleryItems() {
    try {
        // Load the JSON file for artwork image files
        let response = await fetch('assets/data/aw.json');
        let awImages = await response.json();
        
        // Create gallery items
        awImages.forEach(element => {
            CreateGalleryImages(element.file_name, element.url);
        });
